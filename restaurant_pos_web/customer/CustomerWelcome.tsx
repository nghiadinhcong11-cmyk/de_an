...
export function CustomerWelcome() {
  ...
  return (
    <div className="space-y-16 pb-12 p-4 md:p-0">
      {/* Hero Section */}
      <div className="relative rounded-[40px] md:rounded-[50px] overflow-hidden bg-gray-900 text-white p-6 md:p-20">
        ...
        <div className="relative z-10 max-w-2xl space-y-6">
          ...
          <div className="pt-4 md:pt-8 flex flex-col sm:flex-row gap-4">
             <Button onClick={() => navigate('/customer/menu')} className="bg-orange-600 hover:bg-orange-700 h-14 md:h-16 px-8 rounded-2xl font-black text-lg gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
         <div className="space-y-6">
            ...
            <div className="flex gap-3 bg-white p-2 md:p-3 rounded-[24px] shadow-xl border border-gray-50">
               <Input
                 placeholder="Nhập mã bàn..."
                 className="h-12 md:h-14 border-none bg-transparent font-bold text-lg focus-visible:ring-0"
                 value={tableId}
                 onChange={(e: any) => setTableId(e.target.value)}
               />
               <Button onClick={handleGoToTable} className="bg-gray-900 h-12 md:h-14 w-12 md:w-14 rounded-2xl p-0 shrink-0">
                  <ArrowRight className="w-6 h-6" />
               </Button>
            </div>
         </div>
         <div className="bg-orange-50 rounded-[40px] p-8 md:p-10 flex flex-col items-center text-center space-y-4 border-2 border-orange-100/50">
            <div className="w-16 md:w-20 h-16 md:h-20 bg-white rounded-3xl flex items-center justify-center shadow-lg text-orange-600">
               <QrCode className="w-8 md:w-10 h-8 md:h-10" />
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
         <MemberCard
            ...
            title="Điểm tích lũy"
            value={`${(user.points || 0).toLocaleString("vi-VN")} điểm`}
